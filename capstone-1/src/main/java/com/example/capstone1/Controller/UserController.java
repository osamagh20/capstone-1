...
public class UserController {
    ...
    @PutMapping("/recover-product/{idu}/{idp}/{idm}")
    public ResponseEntity recoverProduct(@PathVariable String idu,@PathVariable String idp,@PathVariable String idm){
        String buyResponse = userService.recoverProduct(idu,idp,idm);
         ...
         if (buyResponse.equalsIgnoreCase("user id not found")) {
            ...
            return ResponseEntity.status(400).body("Balance less than price");
        }else if (buyResponse.equalsIgnoreCase("Done Recovery")){
            return ResponseEntity.status(200).body("you buy successful");
        }
        return ResponseEntity.status(400).body("buy product not working please again");
    }
